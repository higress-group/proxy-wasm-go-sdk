...
// impl internal.ProxyWasmHost
func (r *rootHostEmulator) ProxyRedisCall(upstreamData *byte, upstreamSize int, queryData *byte, querySize int, calloutIDPtr *uint32) internal.Status {
	upstream := internal.RawBytePtrToString(upstreamData, upstreamSize)
	...
	log.Printf("[redis call] calloutID: %v", calloutIDPtr)
